...
class TestAgentConfigs:
    ...
    def test_all_agent_types_have_required_fields(self):
        """Every agent config should have tools, mcp_servers, pfactory_tools, thinking_default."""
        from agents.tools_pkg.models import AGENT_CONFIGS

        required_fields = ["tools", "mcp_servers", "pfactory_tools", "thinking_default"]
    ...
    def test_tools_are_lists(self):
        ...
        for agent_type, config in AGENT_CONFIGS.items():
            ...
            assert isinstance(
                config["pfactory_tools"], list
            ), f"Agent '{agent_type}' pfactory_tools should be list"
            assert isinstance(
...
class TestGetRequiredMcpServers:
    ...
    def test_coder_has_context7_and_magestic_ai(self):
        """coder should require context7 and pfactory."""
        from agents.tools_pkg.models import get_required_mcp_servers

        servers = get_required_mcp_servers("coder")
        assert "context7" in servers
        assert "pfactory" in servers
...
class TestGetAllowedTools:
    ...
    def test_qa_reviewer_has_write_for_reports(self):
        """QA reviewer needs Write/Edit to create qa_report.md and update test_plan.json."""
        from agents.tools_pkg.permissions import get_allowed_tools
        ...
        assert "Edit" in tools  # Needs to edit test_plan.json
