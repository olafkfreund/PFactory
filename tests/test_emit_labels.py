...
def _epic() -> EpicPlan:
    return EpicPlan(
        ...
        cost_estimate=CostEstimate(
            monthly_usd=2492.58, confidence="medium", source="aws-price-list"
        ),
        access_requirements=[
            AccessRequirement(provider="aws", action="eks:CreateCluster", granted=True)
        ],
    )
...
def _review(*, blocking=False, passed=True) -> PlanReview:
    ...
    r = PlanReview(
        plan_id="001-orders", lenses=[LensScore(lens="security", score=0.9, findings=findings)]
    )
...
def test_meta_block_is_machine_readable():
    ...
    # RFC-0014: dev-day effort sizing is gone; the effort-free verdict is surfaced.
    assert "effort_points" not in block
