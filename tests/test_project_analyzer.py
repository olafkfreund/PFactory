...
class TestProjectAnalyzerInitialization:
    ...
    def test_get_profile_path_without_spec(self, temp_dir: Path):
        ...
        assert path.resolve() == (temp_dir / ".pfactory" / ".pfactory-security.json").resolve()
    ...
    def test_get_profile_path_with_spec(self, temp_dir: Path, spec_dir: Path):
        ...
        path = analyzer.get_profile_path()
        # Use resolve() to handle /var -> /private/var symlinks on macOS
        assert path.resolve() == (spec_dir / ".pfactory-security.json").resolve()
...
class TestCustomAllowlist:
    ...
    def test_loads_custom_allowlist(self, temp_dir: Path):
        """Loads commands from .pfactory-allowlist."""
        allowlist = """# Custom commands
my-custom-tool
another-command
"""
        (temp_dir / ".pfactory-allowlist").write_text(allowlist)
...
class TestSecurityProfileGeneration:
    ...
    def test_profile_caching(self, python_project: Path):
        ...
        profile_path = python_project / ".pfactory" / ".pfactory-security.json"
...
class TestNpmScriptRunners:
    ...
    def test_any_npm_script_enables_the_npm_family_runners(self, temp_dir: Path):
        (temp_dir / "package.json").write_text(json.dumps({"scripts": {"build": "tsc"}}))
    ...
    def test_the_runner_set_does_not_depend_on_how_many_scripts_there_are(
        ...
        (temp_dir / "package.json").write_text(
            json.dumps({"scripts": {"build": "tsc", "test": "vitest", "lint": "eslint"}})
        )
