"""Tests for the feasibility engine — cost / time / access (Phase C).

The deterministic logic (effort rollup, resource extraction, static-fallback
pricing, IAM action-mapping, the orchestrator) is exercised without any live
cloud. Live AWS pricing/IAM paths are guarded in the modules and only run with
real credentials (mark such tests with ``live_cloud``).
"""
...
from plan.feasibility.effort import estimate_effort  # noqa: E402
...
def test_effort_rolls_up_complexity():
    epic = _epic([
        ChildIssue(key="C1", title="a", complexity="simple"),
        ChildIssue(key="C2", title="b", complexity="standard"),
        ChildIssue(key="C3", title="c", complexity="complex"),
    ])
    est = estimate_effort(epic)
    assert est.story_points == 1 + 3 + 8
    assert est.duration_days_high >= est.duration_days_low > 0
    assert est.assumptions


def test_effort_empty_epic():
    est = estimate_effort(_epic([]))
    assert est.story_points == 0
...
def test_extract_resources_from_text_and_enrichment():
    infra = [{"adapter": "aws", "resources": {"instance_types": {"m5.large": 3}, "regions": ["eu-west-2"]}}]
...
def test_estimate_cost_uses_real_client_when_available():
    plan = _plan("EC2 fleet of m5.large", infra=[
        {"adapter": "aws", "resources": {"instance_types": {"m5.large": 2}, "regions": ["us-east-1"]}}
    ])
...
def test_assess_feasibility_bundles_everything():
    ...
    assert result.effort is not None and result.effort.story_points > 0
    sources = {f.source for f in result.findings}
    assert "feasibility-cost" in sources
    assert "feasibility-effort" in sources
