...
- **Cross-repo schema-drift guard.** The vendored Task Contract schema is now fully synced with the canonical Factory hub copy, and a new CI step (`scripts/check_schema_drift.py`) fails the build if it drifts again (directional subset check; soft-skips on a network error fetching the canonical schema).
