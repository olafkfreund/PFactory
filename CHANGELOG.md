...
- **Delta-aware, grounded plans.** `recon/delta.py` maps acceptance criteria and child issues to **real files**, so subtasks carry populated `files_to_modify`/`files_to_create`/`patterns_from` and `tfactory.ac_to_code_map` is pre-seeded; the emitted contract gains a `baseline` block (RepoMap summary + blast radius) and `provenance.base_ref`/`baseline_commit`. The planning **language now comes from the repo** rather than plan-text keywords, fixing the wrong-language trap (#585) at the source.
- **`change_mode` + language reconciliation.** `recon/change_mode.py` classifies greenfield/modify/migration from the RepoMap; `recon/language_reconcile.py` + a hard `language-reconciled` readiness check HALT a non-migration spec/repo language mismatch instead of silently producing the repo's language. A new `change-footprint-surfaced` check hard-fails a modify/migration plan whose repo was never read. A stable recon digest folds into the approval hash, so sign-off invalidates on repo drift but stays idempotent on the same commit.
- **Language-migration planning.** `detect/migration_classifier.py` recognises a directional rewrite ("port X from Python to Rust"); `detect/source_inspector.py` extracts the behavioral contract (public API + signatures, tests, import DAG) by AST only; `decompose/migration_planner.py` produces the module map, the golden-corpus manifest (declared by the planner; outputs captured later in TFactory's sandbox), and the `tfactory.equivalence` block. `emit/migration_block.py` records `environment.source_language`/`target_language` + the equivalence lane.
- **Cross-repo schema-drift guard.** The vendored Task Contract schema is now fully synced with the canonical Factory hub copy, and a new CI step (`scripts/check_schema_drift.py`) fails the build if it drifts again (directional subset check). It soft-skips only on a *transient* fetch failure (timeout/DNS), and does so loudly (banner + GitHub `::warning` annotation); a deterministic fetch failure — TLS/certificate error, 404, malformed JSON — fails the gate, because it recurs every run and a silent skip would leave the gate permanently green without ever running (#440, Factory#433).
...
- **The evidence gate no longer fails a successful trusted-plan handoff (#143).** #141 downgraded any `emitted` plan with no GitHub epic to `failed` — wrong for the trusted-plan / contract-sync path, where `emit-contract` signs the Task Contract straight to AIFactory and starts the build, creating **no GitHub issues by design**. The gate ignored `session.aifactory_task_id` (the started build, which is the evidence), so every issue-less contract hand-off surfaced as a false `plan stage status='failed'` anomaly in the CFactory cockpit even though the plan passed all gates and the build was running. Now a present `aifactory_task_id` counts as evidence (`proof_kind: "contract"`); a truly empty emit (no epic AND no build) still downgrades to `failed`.
- **Pin `fastapi==0.136.3` / `starlette==1.3.1` (#143).** PFactory was unpinned at `fastapi>=0.109.0`, so CI pulled `0.137.0`, which breaks route introspection (`prometheus get_route_name` → `_IncludedRouter` has no `.path`) and failed 51 route tests; it would also 500 every `/api` route on a fresh deploy. Same regression + pin TFactory and AIFactory shipped — closes it fleet-wide and shields PFactory's next image rebuild.
