...
def _declared_width(model: type, column: str) -> int:
...
def test_resource_id_fits_a_github_backed_composite_task_id() -> None:
    ...
    assert len(task_id) <= _declared_width(AuditLog, "resource_id"), (
        f"resource_id is {_declared_width(AuditLog, 'resource_id')} chars but a "
...
def test_resource_id_is_at_least_as_wide_as_resource_type() -> None:
    ...
    assert _declared_width(AuditLog, "resource_id") >= _declared_width(AuditLog, "resource_type")
