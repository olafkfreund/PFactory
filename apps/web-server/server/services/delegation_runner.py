...
2. **Await** that subprocess to completion (gap #3 from issue #144 — the
   ...
   was written).
3. Render the PFactory enrichment comment.
...
ENRICHMENT_MARKER = "## ✨ PFactory enrichment"
    ...
    *,
    ...
    if proc is not None:
        try:
            ...
            logger.warning(
                "[delegation_runner] planner timed out after %ss task=%s — posting comment with "
    ...
    try:
        ...
        if existing is not None:
            logger.info(
                "[delegation_runner] enrichment comment already exists on issue=%s (id=%s) — "
                "skipping re-post",
    ...
    try:
        ...
        logger.warning(
            "[delegation_runner] provider does not support assign_to_user; skipping Copilot "
