...
export interface Subtask {
  id: string;
  title: string;
  description?: string;  // Optional to match backend's Optional[str]
...
export interface TaskLogs {
  ...
  /** Full build log content from build-progress.txt (for archived/done tasks) */
  buildProgress?: string;
}

// Streaming markers from Python (similar to InsightsStreamChunk)
export interface TaskLogStreamChunk {
  type: TaskLogEntryType;  // Same as TaskLogEntry for consistency
  content?: string;
  phase?: TaskLogPhase;
  timestamp?: string;
  tool?: {
...
export interface TaskDraft {
   ...
   * PFactory still runs the planner phase locally (so the spec gets
...
// Implementation Plan (from pfactory)
...
export interface WorktreeDiffFile {
  ...
  additions: number;
  deletions: number;
  /** The actual git diff content for this file */
  diff?: string;
}
...
export interface GitConflictInfo {
  ...
  totalRenames?: number;
  // Whether there's an active git merge in progress with conflict markers
  // (MERGE_HEAD exists and files have <<<<<<< markers)
...
export interface WorktreeMergeResult {
  ...
  preview?: {
    ...
    uncommittedChanges?: {
      ...
      files: string[];
      count: number;
      // Files that overlap with task branch (actual conflicts)
      conflictingFiles?: string[];
      // True if any files conflict with task branch
      hasConflicts?: boolean;
    } | null;
  };
}
