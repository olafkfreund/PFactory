...
export interface ProjectSettings {
   ...
   * ``claude auth login`` on the PFactory host.
   */
...
export interface GraphitiProviderConfig {
  ...
  database?: string;  // Database name (default: pfactory_memory)
  dbPath?: string;    // Database storage path (default: ~/.pfactory/memories)
}
 ...
 * Stored in .pfactory/.env as AGENT_MCP_<agent>_ADD and AGENT_MCP_<agent>_REMOVE
 */
...
export interface AutoBuildVersionInfo {
  ...
  updateAvailable: boolean; // Always false - .pfactory only contains data, no code to update
}
...
export interface FileNode {
  ...
  name: string;
  isDirectory: boolean;
  type?: 'file' | 'directory'; // Web API adds this field
  children?: FileNode[];
}
