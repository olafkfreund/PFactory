...
export const useSettingsStore = create<SettingsState>((set) => ({
  ...
  updateProfile: async (profile: APIProfile): Promise<boolean> => {
    set({ profilesLoading: true, profilesError: null });
    try {
      const result = await window.API.updateAPIProfile(profile);
      if (result.success && result.data) {
        set((state) => ({
  ...
  deleteProfile: async (profileId: string): Promise<boolean> => {
    set({ profilesLoading: true, profilesError: null });
    try {
      const result = await window.API.deleteAPIProfile(profileId);
      if (result.success) {
        set((state) => ({
  ...
  setActiveProfile: async (profileId: string | null): Promise<boolean> => {
    set({ profilesLoading: true, profilesError: null });
    try {
      const result = await window.API.setActiveAPIProfile(profileId);
      if (result.success) {
  ...
  testConnection: async (baseUrl: string, apiKey: string, signal?: AbortSignal): Promise<TestConnectionResult | null> => {
    ...
    try {
      const result = await window.API.testConnection(baseUrl, apiKey, signal);
  ...
  discoverModels: async (baseUrl: string, apiKey: string, signal?: AbortSignal): Promise<ModelInfo[] | null> => {
    ...
    try {
      console.log('[settings-store] Calling window.API.discoverModels...');
      const result = await window.API.discoverModels(baseUrl, apiKey, signal);
...
export async function saveSettings(updates: Partial<AppSettings>): Promise<boolean> {
  ...
  try {
    const result = await window.API.saveSettings(updates);
    if (result.success) {
...
export async function loadProfiles(): Promise<void> {
  ...
  try {
    const result = await window.API.getAPIProfiles();
    if (result.success && result.data) {
