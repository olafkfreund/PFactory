...
interface ProjectState {
  ...
  isLoading: boolean;
  error: string | null;
  isSwitchingProject: boolean; // Loading state for project switching
...
export const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  closeProjectTab: (projectId) => {
    ...
    const newSelectedProjectId = newActiveProjectId;
    if (newSelectedProjectId) {
      localStorage.setItem(LAST_SELECTED_PROJECT_KEY, newSelectedProjectId);
    } else {
      localStorage.removeItem(LAST_SELECTED_PROJECT_KEY);
    }
...
function saveTabStateToMain(): void {
  ...
  saveTabStateTimeout = setTimeout(async () => {
    ...
    try {
      await window.API.saveTabState(tabState);
    } catch (err) {
...
export async function loadProjects(): Promise<void> {
  ...
  try {
    ...
    const tabStateResult = await window.API.getTabState();
 ...
 * ``~/.pfactory/workspaces/`` on laptops; a PVC on K8s) and registers
...
export async function removeProject(projectId: string): Promise<boolean> {
  ...
  try {
    const result = await window.API.removeProject(projectId);
    if (result.success) {
...
export async function updateProjectSettings(
  ...
  try {
    const result = await window.API.updateProjectSettings(
      projectId,
      settings
    );
    if (result.success) {
...
/**
 * Check pfactory version status for a project
 */
export async function checkProjectVersion(
  projectId: string
): Promise<AutoBuildVersionInfo | null> {
  try {
    const result = await window.API.checkProjectVersion(projectId);
    if (result.success && result.data) {
      return result.data;
    }
    return null;
...
/**
 * Initialize pfactory in a project
 */
export async function initializeProject(
  ...
  try {
    ...
    const result = await window.API.initializeProject(projectId);
    ...
    if (result.success && result.data) {
      ...
      if (result.data.success) {
        console.log('[ProjectStore] Updating project autoBuildPath to .pfactory');
        store.updateProject(projectId, { autoBuildPath: '.pfactory' });
      } else {
