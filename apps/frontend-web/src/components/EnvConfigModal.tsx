...
export function EnvConfigModal({
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const [tokenResult, profilesResult] = await Promise.all([
          window.API.checkSourceToken(),
          window.API.getClaudeProfiles()
        ]);
  ...
  useEffect(() => {
    ...
    const cleanup = window.API.onTerminalOAuthToken(async (info) => {
      if (info.success) {
  ...
  const handleUseExistingProfile = async () => {
    ...
    try {
      ...
      // Save the token to pfactory .env
  ...
  const handleAuthenticateWithBrowser = async () => {
    ...
    try {
      ...
      const result = await window.API.invokeClaudeSetup(projectId);
                ...
                <div className="space-y-3 pl-4 border-l-2 border-border">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      <code className="px-1 py-0.5 bg-muted rounded font-mono">
                        {sourcePath ? `${sourcePath}/.env` : 'pfactory/.env'}
                      </code>
                    </p>
                  </div>
                </div>
...
export function useClaudeTokenCheck() {
  ...
  const checkToken = async () => {
    ...
    try {
      const result = await window.API.checkSourceToken();
