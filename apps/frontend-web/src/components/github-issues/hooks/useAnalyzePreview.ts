...
export function useAnalyzePreview({ projectId }: UseAnalyzePreviewProps): UseAnalyzePreviewReturn {
  ...
  useEffect(() => {
    ...
    const cleanupProgress = window.API.github.onAnalyzePreviewProgress(
    ...
    const cleanupComplete = window.API.github.onAnalyzePreviewComplete(
    ...
    const cleanupError = window.API.github.onAnalyzePreviewError(
  ...
  const startAnalysis = useCallback(() => {
    ...
    window.API.github.analyzeIssuesPreview(projectId, undefined, 200);
  }, [projectId]);
  ...
  const approveBatches = useCallback(async (batches: ProposedBatch[]) => {
    ...
    setIsApproving(true);
    try {
      const result = await window.API.github.approveBatches(projectId, batches);
      if (!result.success) {
