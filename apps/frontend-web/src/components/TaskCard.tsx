...
function taskCardPropsAreEqual(prevProps: TaskCardProps, nextProps: TaskCardProps): boolean {
  ...
  const isEqual = (
    ...
    prevTask.specId === nextTask.specId &&
    ...
    prevTask.executionProgress?.overallProgress === nextTask.executionProgress?.overallProgress &&
    ...
    prevTask.metadata?.mode === nextTask.metadata?.mode &&
  ...
  if (window.DEBUG && !isEqual) {
    ...
    if (prevTask.executionProgress?.overallProgress !== nextTask.executionProgress?.overallProgress) {
      changes.push(`progress: ${prevTask.executionProgress?.overallProgress}% -> ${nextTask.executionProgress?.overallProgress}%`);
    }
...
export const TaskCard = memo(function TaskCard({ task, onClick }: TaskCardProps) {
  ...
  // Memoize task number for styling
  const taskNumber = useMemo(
    () => extractTaskNumber(task.specId),
    [task.specId]
  );

  // Formatted title for tooltip
  const formattedTitle = useMemo(
    () => formatTaskTitleWithNumber(task.title, task.specId),
  ...
  const handleRecover = async (e: React.MouseEvent) => {
    ...
    if (result.success) {
      ...
      if (result.autoRestarted) {
        toast({ title: t('status.recovered'), description: 'Task recovered and restarted successfully' });
      } else if (result.autoRestartError) {
        toast({
          variant: 'destructive',
          title: t('status.recovered'),
          description: `Task recovered but restart failed: ${result.autoRestartError}`
        });
      } else {
        toast({ title: t('status.recovered'), description: 'Task recovered and reset to backlog' });
      }
    } else {
      toast({ variant: 'destructive', title: 'Recovery Failed', description: result.message || 'Failed to recover task' });
    }
  ...
  const handleArchive = async (e: React.MouseEvent) => {
    ...
    if (!result.success) {
      toast({ variant: 'destructive', title: t('kanban.archiveFailed'), description: result.error });
    } else {
      toast({ title: t('status.archived'), description: task.title });
    }
  };
        ...
        {(task.metadata || isStuck || isIncomplete || hasActiveExecution || reviewReasonInfo) && (
          <div className="mt-2.5 flex flex-wrap gap-1.5">
            ...
            {task.metadata?.mode === 'quick' && (
              <Badge
                variant="outline"
                className="text-[10px] px-1.5 py-0.5 flex items-center gap-1 bg-amber-500/10 text-amber-400 border-amber-500/30"
              >
