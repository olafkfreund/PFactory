...
def display_plan_summary(spec_dir: Path) -> None:
    """
    Display summary of test_plan.json for human review.
    ...
    try:
        ...
        print_status(f"Could not read test_plan.json: {e}", "error")
        return
...
def offer_html_plan_view(spec_dir: Path, auto_open: bool = False) -> bool:
    """Offer to generate and open an HTML view of the plan.
    ...
    try:
        ...
        print()
        content = [
            f"{icon(Icons.DOCUMENT)} {highlight('Interactive HTML Plan Available')}",
            "",
            "A more readable HTML version has been generated:",
            f"  {muted('File:')} {html_path.name}",
            "",
        ]
        ...
        print_status(f"Could not generate HTML plan: {e}", "warning")
        return False
