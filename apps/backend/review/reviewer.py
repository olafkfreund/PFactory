...
class ReviewChoice(Enum):
    ...
    EDIT_PLAN = "edit_plan"  # Edit test_plan.json
...
def get_review_menu_options() -> list[MenuOption]:
    ...
    return [
        ...
        MenuOption(
            ...
            icon=Icons.DOCUMENT,
            description="Open test_plan.json in your editor",
        ),
        MenuOption(
...
def run_review_checkpoint(
    ...
    # Bug #4 Fix: Generate HTML and save review state BEFORE interactive loop
    # This allows web UI to detect review checkpoint even in CI mode
    offer_html_plan_view(spec_dir, auto_open=False)

    # Save review state with approved=false for web UI
    if not state.approved:
        state.save(spec_dir)

    # Check if running in CI mode (non-interactive)
    is_ci_mode = os.environ.get("CI", "").lower() == "true"

    if is_ci_mode:
        # In CI mode, we've created the files (plan_review.html, review_state.json)
        # Now exit gracefully so web UI can handle the approval
        print()
        content = [
            warning(f"{icon(Icons.INFO)} AWAITING APPROVAL"),
            "",
            "Spec creation complete. Manual review required.",
            "",
            "The web UI will display the plan for your approval.",
            f"Spec directory: {spec_dir.name}",
        ]
        print(box(content, width=70, style="light"))
        print()
        # Don't raise SystemExit - just return the unapproved state
        # The caller (orchestrator) will handle this gracefully
        return state

    # Main review loop with graceful Ctrl+C handling (interactive mode only)
    try:
        while True:
            ...
            if choice == ReviewChoice.APPROVE.value:
                ...
                if not plan_file.exists():
                    print_status("test_plan.json not found", "error")
                    continue
