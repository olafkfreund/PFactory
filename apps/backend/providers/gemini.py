...
#
# PFactory will add an Antigravity provider once the SDK stabilizes past
...
class GeminiCLIProvider(BaseLLMProvider):
    ...
    async def _run_gemini(self) -> AsyncGenerator[Any, None]:
        ...
        if not self._pending_prompt:
            logger.warning(
                "GeminiCLIProvider.receive_response() called before query() — no prompt to send"
            )
            return
        ...
        logger.debug(
            "GeminiCLIProvider: subprocess finished returncode=%d stdout_len=%d stderr_len=%d",
            proc.returncode,
            len(stdout_text),
            len(stderr_text),
        )
