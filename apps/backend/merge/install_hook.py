...
Usage:
    python -m magestic_ai.merge.install_hook [--project-path /path/to/project]
"""
...
# Skip if we're in a worktree (pfactory branches)
if [[ "$BRANCH" == "main" ]] || [[ "$BRANCH" == "master" ]]; then
    ...
    if [[ -d ".git" ]]; then
        ...
        ($PYTHON -m magestic_ai.merge.tracker_cli notify-commit "$COMMIT_HASH" 2>/dev/null &) &
