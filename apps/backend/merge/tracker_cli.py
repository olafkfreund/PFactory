...
Usage:
    python -m magestic_ai.merge.tracker_cli notify-commit <hash>
    python -m magestic_ai.merge.tracker_cli show-timeline <file_path>
    python -m magestic_ai.merge.tracker_cli show-drift <task_id>
"""
...
def find_project_root() -> Path:
    """Find the project root by looking for .pfactory or .git directory."""
    current = Path.cwd()

    # Walk up until we find .pfactory or .git
    while current != current.parent:
        if (current / ".pfactory").exists() or (current / ".git").exists():
            return current
