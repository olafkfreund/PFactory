...
```json
{
  ...
  "target_name": "<.pfactory.yml target name, or null if no target declared>",
...
### Step 2 — Check `pfactory_yml.json` targets (injected above)

If no catalog hit, examine the PFACTORY YML targets. Match targets to the
...
9. **`target_name`** — `.pfactory.yml` target name if Step 2 matched; otherwise `null`.
...
- `{spec_dir}/context/pfactory_yml.json` — declared `.pfactory.yml`
...
5. **Read** `context/pfactory_yml.json` (if present) — identify targets.
