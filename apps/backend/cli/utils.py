...
def setup_environment() -> Path:
    ...
    """
    # Add pfactory directory to path for imports
    script_dir = Path(__file__).parent.parent.resolve()
    sys.path.insert(0, str(script_dir))

    # Load .env file - check both pfactory/ and dev/pfactory/ locations
    env_file = script_dir / ".env"
    dev_env_file = script_dir.parent / "dev" / "pfactory" / ".env"
    if env_file.exists():
