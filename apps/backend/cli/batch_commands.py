...
def handle_batch_create_command(batch_file: str, project_dir: str) -> bool:
    ...
    specs_dir = Path(project_dir) / ".pfactory" / "specs"
...
def handle_batch_status_command(project_dir: str) -> bool:
    ...
    for spec_dir in specs:
        ...
        if (spec_dir / "spec.md").exists():
            status = "spec_created"
        elif (spec_dir / "test_plan.json").exists():
            status = "building"
...
def handle_batch_cleanup_command(project_dir: str, dry_run: bool = True) -> bool:
    ...
    """
    specs_dir = Path(project_dir) / ".pfactory" / "specs"
    worktrees_dir = Path(project_dir) / ".pfactory" / "worktrees" / "tasks"
    ...
    if dry_run:
        ...
        for spec_name in completed:
            ...
            if wt_path.exists():
                print(f"    └─ .pfactory/worktrees/tasks/{spec_name}/")
        print()
