...
ccstatusline Configuration:
    Add to ~/.config/ccstatusline/settings.json:
    {
        "widgets": [
            {
                "type": "custom_command",
                "command": "python /path/to/pfactory/statusline.py",
                "refresh": 5000
            }
        ]
    }
...
def find_project_root() -> Path:
    """Find the project root by looking for .pfactory or .pfactory-status."""
    cwd = Path.cwd()

    # Check current directory - prioritize .pfactory (installed instance)
    if (cwd / ".pfactory").exists():
        return cwd
    if (cwd / ".pfactory-status").exists():
        return cwd

    # Walk up to find project root
    for parent in cwd.parents:
        if (parent / ".pfactory").exists():
            return parent
        if (parent / ".pfactory-status").exists():
            return parent
