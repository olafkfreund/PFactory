...
def get_relative_spec_path(spec_dir: Path, project_dir: Path) -> str:
    ...
    Returns:
        Relative path string (e.g., "./pfactory/specs/003-new-spec")
    """
    try:
        ...
        return f"./pfactory/specs/{spec_dir.name}"
...
- Plan: `{relative_spec}/test_plan.json`
...
5. **Commit your changes:**
   ```bash
   git add . ':!.pfactory'
   git commit -m "pfactory: {subtask_id} - {description[:50]}"
   ```
6. **Update the plan** - set this subtask's status to "completed" in test_plan.json
...
def generate_planner_prompt(spec_dir: Path, project_dir: Path | None = None) -> str:
    ...
    if project_dir is None:
        # Infer: spec_dir is typically project/pfactory/specs/XXX
...
- `{relative_spec}/test_plan.json` - Subtask-based implementation plan
