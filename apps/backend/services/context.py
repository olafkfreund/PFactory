...
Usage:
    ...
    python pfactory/service_context.py --service backend --output backend/SERVICE_CONTEXT.md

    # Generate for all services (using project index)
    python pfactory/service_context.py --all

    # Generate with custom project index
    python pfactory/service_context.py --service frontend --index pfactory/project_index.json
"""
...
class ServiceContextGenerator:
    ...
    def _load_project_index(self) -> dict:
        """Load project index from file (.pfactory is the installed instance)."""
        index_file = self.project_dir / ".pfactory" / "project_index.json"
