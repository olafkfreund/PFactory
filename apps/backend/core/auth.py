"""
Authentication helpers for PFactory.

Provides centralized authentication token resolution with fallback support
for multiple environment variables, PFactory profiles, Claude Code CLI
...
def _get_token_from_pfactory_profiles() -> str | None:
    """Read token from PFactory profiles storage (~/.pfactory/claude-profiles.json)."""
    path = Path.home() / ".pfactory" / "claude-profiles.json"
    if not path.exists():
        return None
    try:
...
def get_auth_token() -> str | None:
    ...
    3. PFactory profiles (~/.pfactory/claude-profiles.json)
    ...
    # Check PFactory profiles
    token = _get_token_from_pfactory_profiles()
    if token:
        return token
...
def get_auth_token_source() -> str | None:
    ...
    # Check PFactory profiles
    if _get_token_from_pfactory_profiles():
        return "PFactory Profiles"
