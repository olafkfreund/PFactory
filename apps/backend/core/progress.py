...
Uses subtask-based implementation plans (test_plan.json).
...
def count_subtasks(spec_dir: Path) -> tuple[int, int]:
    """
    Count completed and total subtasks in test_plan.json.

    Args:
        spec_dir: Directory containing test_plan.json
...
def print_progress_summary(spec_dir: Path, show_next: bool = True) -> None:
    ...
    if total > 0:
        ...
        # Phase summary
        try:
            with open(spec_dir / "test_plan.json") as f:
                plan = json.load(f)
...
def print_build_complete_banner(spec_dir: Path) -> None:
    ...
    content = [
        ...
        "",
        muted("Next steps:"),
        f"  1. Review the {highlight('pfactory/*')} branch",
        "  2. Run manual tests",
