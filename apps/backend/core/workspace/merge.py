#!/usr/bin/env python3
"""
Workspace merge operations (intent-aware merge, conflict resolution, AI merge).

Part of the ``core.workspace`` package, alongside:
...
This file used to live at ``core/workspace.py``, BESIDE the package rather than
inside it — where Python could not load it at all, because a package always wins
over a same-named module. ``__init__.py`` reached around that with
``importlib.util.spec_from_file_location`` and executed the file under the
separate name ``workspace_module``, so 55K of merge logic ran under a second
module identity that no static tool could follow: mypy, ruff's import rules,
IDE go-to-definition and every dead-code check saw a file nothing imported,
while a whole-package mypy run aborted on the duplicate name having checked
nothing. Moving it inside the package makes the import ordinary (PFactory#477).
"""
...
def merge_existing_build(
    ...
    Called when user runs: python pfactory/run.py --spec X --merge
    ...
    if not worktree_path:
        ...
        print(highlight(f"  python pfactory/run.py --spec {spec_name}"))
        return False
    ...
    spec_branch = f"pfactory/{spec_name}"

    # Don't merge a branch into itself
    if current_branch == spec_branch:
        print()
        print_status("You're on the spec branch. Switch to your target branch first.", "warning")
        print()
        print("Example:")
        ...
        if smart_result is not None:
            ...
            if smart_result.get("success"):
                ...
                if had_conflicts or files_merged or ai_assisted:
                    ...
                    # User can delete with: python pfactory/run.py --spec <name> --discard
    ...
    success_result = manager.merge_worktree(spec_name, delete_after=False, no_commit=no_commit)
    ...
    if success_result:
        ...
        if no_commit:
            ...
            print(muted(f"  python pfactory/run.py --spec {spec_name} --discard"))
        else:
    ...
    try:
        ...
        if merge_tree_result.returncode != 0:
            ...
            for line in output.split("\n"):
                if "CONFLICT" in line:
                    ...
                    if match:
                        ...
                        # Skip .pfactory files - they should never be merged
                        if (
                            file_path
                            and file_path not in result["conflicting_files"]
                            and not _is_magestic_ai_file(file_path)
                        ):
            ...
            if not result["conflicting_files"]:
                ...
                # Filter out .pfactory files - they should never be merged
...
def _resolve_git_conflicts_with_ai(
    ...
    spec_branch = git_conflicts.get("spec_branch", f"pfactory/{spec_name}")
    ...
    if merge_base:
        ...
        if path_mappings:
            ...
            print(muted(f"  Detected {len(path_mappings)} file rename(s) since branch creation"))
    ...
    for file_path in conflicting_files:
        ...
        try:
            ...
            main_content = _get_file_content_from_ref(project_dir, base_branch, target_file_path)
    ...
    simple_copy_files: list[tuple[str, str, str]] = []  # (file_path, target_path, status)
    ...
    for file_path, status in non_conflicting:
        ...
        if target_file_path != file_path and status != "D":
            ...
            worktree_content = _get_file_content_from_ref(project_dir, spec_branch, file_path)
            target_content = _get_file_content_from_ref(project_dir, base_branch, target_file_path)
            base_content = None
            if merge_base:
                base_content = _get_file_content_from_ref(project_dir, merge_base, file_path)
