...
def choose_workspace(
    ...
    """
    Let user choose where pfactory should work.
...
def copy_spec_to_worktree(
    ...
    # Use .pfactory/specs/{spec_name}/ as the standard location
    # Note: pfactory/ is source code, .pfactory/ is the installed instance
    target_spec_dir = worktree_path / ".pfactory" / "specs" / spec_name
...
def initialize_timeline_tracking(
    ...
    try:
        ...
        if source_spec_dir:
            plan_path = source_spec_dir / "test_plan.json"
            if plan_path.exists():
