...
> Design document for converting PFactory from Electron to a containerized web application.

## Executive Summary

This document outlines the architecture for a Docker-native version of PFactory that:
...
│  │  - Reverse proxy: /api/* → FastAPI :3114                            ││
│  │  - WebSocket proxy: /ws/* → FastAPI :3114                           ││
...
| POST | `/api/projects/{id}/initialize` | Initialize pfactory in project |
| GET | `/api/projects/{id}/version` | Check pfactory version |
...
WORKDIR /app
COPY pfactory/requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

# Install API dependencies
COPY pfactory/api/requirements.txt ./api-requirements.txt
RUN pip install --no-cache-dir -r api-requirements.txt

# Copy application code
COPY pfactory/ ./pfactory/

# --- Frontend Build Stage ---
FROM node:22-alpine AS frontend-build

WORKDIR /app
COPY pfactory-ui/package*.json ./
RUN npm ci
...
cd /app/pfactory
uvicorn api.main:app --host 0.0.0.0 --port 3114 &
...
services:
  app:
    ...
    volumes:
      ...
      - pfactory-claude:/home/claude/.claude
    environment:
  ...
  falkordb:
    ...
    container_name: pfactory-falkordb
    volumes:
      - falkordb-data:/data
    networks:
      - pfactory-net
  ...
  graphiti-mcp:
    ...
    container_name: pfactory-graphiti
...
1. OAuth tokens stored in named volume (`pfactory-claude`)
